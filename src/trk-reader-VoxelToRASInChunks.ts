...
let globalHeader: TrkHeader | null = null;

// Function to stream and process the TRK header
async function streamAndProcessHeader(url: string, start: number, end: number) {
    try {
        ...
        // Read the header from the buffer and store it in the global variable
        globalHeader = readTrkHeader(buffer);
        printTrkHeader(globalHeader);
...
function voxel_to_RAS(point: [number, number, number], matrix: number[][]): [number, number, number] {
    ...
    // Apply the matrix multiplication for RAS coordinates
...
// Function to stream and process the TRK file in chunks
async function streamAndProcessTrackDataInChunks(url: string, start: number) {
    try {
        ...
        while (!endReached) {
            ...
            const response = await axios.get(url, {
                responseType: 'arraybuffer',
                headers: {
                    'Range': `bytes=${totalBytes}-${totalBytes + CHUNK_SIZE - 1}`
                },
            ...
            const buffer = Buffer.from(response.data);
            console.log(`\nReceived ${response.data.byteLength} bytes starting from byte ${totalBytes}`);

            // If the received data is less than the chunk size, we've reached the end
            ...
            // Increment the total bytes processed
...
function readTrackDataAndWrite(buffer: Buffer, offset: number, writeStream: fs.WriteStream) {
    ...
    if (!globalHeader) {
        console.error('Error: Header has not been initialized.');
        return;
    }
    ...
    while (offset < buffer.length) {
        ...
        if (trackNumber === 119) {
            ...
            for (let i = 0; i < n_points; i++) {
                ...
                // Perform voxel to RAS conversion using globalHeader
                const rasPoint = voxel_to_RAS(voxelPoint, globalHeader.vox_to_ras);
        ...
        // Write a simple log for tracks other than 119
...
// Update the main function to use the chunked processing
async function main() {
    ...
    // Stream and process the entire track data in chunks
    await streamAndProcessTrackDataInChunks(trkFileUrl, headerChunkSize);
}
