...
// Update streamAndProcessTrackData to pass the header object
async function streamAndProcessTrackData(url: string, start: number) {
    try {
        ...
        const response = await axios.get(url, {
            ...
            maxContentLength: Infinity,  // Allow Axios to handle large content
        ...
        const outputFilePath = path.join(__dirname, 'track_data_VoxelToRAS.txt');
        const writeStream = fs.createWriteStream(outputFilePath);

        // Record the start time for processing
        const processStartTime = new Date().getTime();

        // Process the track data and write to file (access globalHeader inside)
...
// Update the main function to pass the header to the track processing function
async function main() {
