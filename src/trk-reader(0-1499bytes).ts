...
// Function to stream a specific range of bytes from the TRK file and process the header
async function streamAndProcessHeader(url: string, start: number, end: number) {
  try {
    ...
    // Ensure the buffer length is sufficient before processing
...
async function main() {
  ...
  const headerChunkSize = 1500;  // Increase the chunk size to capture the full header

  // Stream and process the header chunk
