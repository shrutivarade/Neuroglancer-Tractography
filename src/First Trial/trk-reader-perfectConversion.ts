...
async function streamAndProcessTrackData(url: string, start: number) {
  try {
    ...
    const outputFilePath = path.join(__dirname, 'track_perfectConversion.txt');
...
function readTrackDataAndWrite(buffer: Buffer, offset: number, writeStream: fs.WriteStream) {
  ...
  while (offset < buffer.length) {
    ...
    if (trackNumber === 69830) {
      writeStream.write(`Track ${trackNumber} processing voxel to RAS conversion for ${n_points} number of points\n`);
