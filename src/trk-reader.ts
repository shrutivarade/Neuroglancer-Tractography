...
// Function to read the next track data from the buffer within the 2000-byte limit
function readTrackDataWithinLimit(buffer: Buffer, offset: number, byteLimit: number) {
  let bytesRead = 0;

  while (offset < buffer.length && bytesRead < byteLimit) {
    ...
    if (bytesRead + bytesForTrack > byteLimit) {
      console.log(`\nReached the byte limit of ${byteLimit} bytes. Stopping further reading.`);
      break;
    }
...
// Function to stream and process the track data within 2000 bytes after the header
async function streamAndProcessTrackDataWithinLimit(url: string, start: number, byteLimit: number) {
  try {
    const response = await axios.get(url, {
      responseType: 'arraybuffer',
      headers: {
        'Range': `bytes=${start}-${start + byteLimit - 1}`
      }
    });

    console.log(`\nReceived ${response.data.byteLength} bytes from range ${start}-${start + byteLimit - 1}`);
    const buffer = Buffer.from(response.data);

    readTrackDataWithinLimit(buffer, 0, byteLimit);
...
async function main() {
  ...
  const byteLimit = 2000; // Limit the size to 2000 bytes

  // Stream and process the track data within the byte limit
  await streamAndProcessTrackDataWithinLimit(trkFileUrl, headerChunkSize, byteLimit);
