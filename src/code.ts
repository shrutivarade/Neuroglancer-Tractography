...
async function downloadTrkFile(url: string, outputPath: string): Promise<Buffer> {
  const response = await axios.get(url, { responseType: 'arraybuffer' });
  fs.writeFileSync(outputPath, Buffer.from(response.data), 'binary');
  console.log('TRK file downloaded to', outputPath);
  return Buffer.from(response.data);
}

// Main function to read and process the TRK file
async function main() {
  ...
  const buffer = await downloadTrkFile(trkFileUrl, trkFilePath);

  // Ensure the buffer has at least the header size
  if (buffer.length < 1000) {
    console.error('Error: The file is too small to be a valid .trk file.');
    return;
  }
